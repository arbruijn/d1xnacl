...
static const char *getFileVal(const char *filename, int *size) {
    const char *p;
    if ((p = strrchr(filename, '/')))
        p++;
    else
        p = filename;
    if (!strcmp(p, "descent.hog")) {
        fprintf(stderr, "ret hog %p, %d s=%p\n", hogdata, hogdataSize, size);
        if (size)
            *size = hogdataSize;
        return hogdata;
    }
    if (!strcmp(p, "descent.pig")) {
        fprintf(stderr, "ret pig %p, %d s=%p\n", pigdata, pigdataSize, size);
        if (size)
            *size = pigdataSize;
        return pigdata;
    }
...
{
    BAIL_IF_MACRO(!isDir(fname) && !getFileVal(fname, NULL), strerror(ENOENT), 0);
    return(1);
...
static int mfRead(void *f, char *buf, int len) {
    struct mem_file *mf = (struct mem_file *)f;
    int ret;
    //char msgbuf[32];
...
static int mfWrite(void *f, const char *buf, int len) {
    struct mem_file *mf = (struct mem_file *)f;
    return -1;
    /*
    int ret;
...
static fpos_t mfSeek(void *f, fpos_t pos, int whence) {
    struct mem_file *mf = (struct mem_file *)f;
    if (whence == SEEK_CUR)
        pos += mf->pos;
...
static void *doOpen(const char *filename, int mode) {
    ...
    struct mem_file *f;
    int size;
    fileVal = getFileVal(filename, &size);
    if (!fileVal) {
        BAIL_IF_MACRO(1, strerror(ENOENT), NULL);
    }
    #undef malloc
    if (!(f = malloc(sizeof *f))) {
        BAIL_IF_MACRO(1, strerror(ENOMEM), NULL);
    }
    f->data = fileVal;
    f->size = size;
    f->pos = 0;
    return funopen(f, mfRead, mfWrite, mfSeek, mfClose);
}
